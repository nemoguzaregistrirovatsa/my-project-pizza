  ...
  $.ajax({
    ...
    success: function(data) {
      for (var key in data) {
        ...
        var button = "";  
        if (data[key].active) {
          button = '<div class="done-div" _id="' + data[key]._id + '">Done</div>'
        }
      ...
      $(".done-div").hover(function(e) {
        $(e.target).css({"cursor": "pointer"});
      });
      
      $(".done-div").click(function(e) {
        $.ajax({
          type: "POST",
          url: "/allorders",
          data: {_id: $(e.target).attr("_id")},
          success: function(data) {
            if (data) $($($(e.target).parent().children()[0]).children()[1]).text("Closed");
